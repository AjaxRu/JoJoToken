...
Ниже продемонстрирована работа функций: <br/>-getBalance(возвращает баланс аккаунта), <br/>-sendTransaction(отправка транзакции).<br/>
Токен ERC-20 создается в момент развертывания смарт-контракта на блокчейне Ethereum. Обычно при создании указывается общее количество токенов (total supply), которое будет доступно для обращения (в нашем случае 1000000000).
